...
const GroupInfoCard = ({data}:any) => {
  ...
  const [profileImg, setProfileImg] = useState(data?.images?.avatar || "/images/article.png")
  const [coverImg, setCoverImg] = useState(data?.images?.cover || "/images/formbg.png")
  ...
  const handleImageUpload =async (item)=>{
    ...
    formData.append("avatar", item=="avatar"?selectedImg.file:selectedCoverImage.file)
    console.log(selectedImg);
    
   
    try{
      const response = await axios.put(`${config.serverUrl}/api/groups/group/${data._id}?imageType=${item}`,
        formData,
        { headers: {
            authorization:`Bearer ${localStorage.getItem('accessToken')}`,
      ...
      setProgress1(false)
      setProgress2(false)
      item=="avatar"?setShowUpdateImgBtn(false):setShowUpdateCoverBtn(false)
  ...
  return (
    <Card className="mb-3">
      <ToastContainer/>
      <input onChange={(e)=>handleImgSelection(e)} name="image" type="file" ref={fileInput} style={{display:'none'}} />
      <input onChange={(e)=>handleImgSelection(e)} name="cover" type="file" ref={coverFileInpt} style={{display:'none'}} />
      <CardImg
        src={coverImg}
        height={150}
      ...
      />
      {
        ...
        overlay={
          <Popover show={false} id={`popover-positioned-bottom`}>
            <Popover.Body style={popOverStyle} onClick={()=>triggerClick(fileInput)}>Select Profile Image</Popover.Body>
            <Popover.Body style={popOverStyle}  onClick={()=>triggerClick(coverFileInpt)}>Select Cover Photo</Popover.Body>
          </Popover>
        }
        show={show}
      >
         <div className={styles.editCoverTab}>
            <span onClick={()=>setShow(!show)}>Edit <AiFillEdit/></span>
            { 
              showUpdateImgBtn &&  
              <Button onClick={()=>handleImageUpload('avatar')} style={{marginLeft:'10px', fontSize:'12px'}}>
                {progress1? <Spinner radius={22} color={"lightgray"} stroke={2} visible={true} />:' update avatar'}
              </Button> 
           }
            { 
              showUpdateCoverBtn &&  
              <Button onClick={()=>handleImageUpload('cover')} style={{marginLeft:'10px', fontSize:'12px'}}>
                 {progress2? <Spinner radius={22} color={"lightgray"} stroke={2} visible={true} />:' update cover'}
              </Button> 
            }
          </div>
      </OverlayTrigger>
