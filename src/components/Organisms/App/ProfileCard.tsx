...
const ProfileCard = ({
  ...
  const [profileImg, setProfileImg] = useState(data?.images?.avatar || "/images/formbg.png")
  ...
  const handleImageUpload =async (item)=>{
    ...
    try{
      const response = await axios.put(`${config.serverUrl}/api/users/${data._id}?imageType=${item}`,
        formData,
        { headers: {
  ...
  return (
    <Card className="mt-2 mb-3">
      <ToastContainer/>
      <Image src={coverImg} className="image3" />
      
        <div className={styles.editCoverTab}>
          <div onClick={()=>triggerClick(coverFileInpt)}>
            <span style={{color:'#32324D'}}>Edit</span> <AiFillEdit size={23} color="#32324D" />
          </div>
           {
            showUpdateCoverBtn &&
            <Button style={{fontSize:'13px', maxWidth:'200px'}} onClick={()=>handleImageUpload('cover')} className={styles.editTab}>
              {!progress2?'Update cover ': <Spinner radius={22} color={"lightgray"} stroke={2} visible={true} />}
            </Button>
           }
      </div>
      ...
      <Card.Body className="d-flex position-relative justify-content-center align-items-center flex-column ">
       ...
       <div onClick={()=>triggerClick(fileInput)} className={styles.editTab}>
        ...
        {
            showUpdateImgBtn &&
            <Button style={{fontSize:'13px', marginTop:'10px'}} onClick={()=>handleImageUpload('avatar')} >
              {!progress1?'Update cover ': <Spinner radius={22} color={"lightgray"} stroke={2} visible={true} />}
            </Button>
        }
